...
export default function AdminPortal() {
  ...
  async function handleTestAPIConfig(item: APIConfig) {
    ...
    setNotice("");
    try {
      const result = await api.adminTestAPIConfig(token, item.id, { arguments: argumentsPayload });
              ...
              {canViewMCPTools && currentMCPTab === "api-tools" ? (
                <article className="content-card">
                  <div className="section-toolbar">
                    <div>
                      <h2>API 接口工具管理</h2>
                      <p className="helper-text">这里维护后台和用户可见的动态 API 工具，新增和编辑都通过弹窗处理，保存后会自动同步进 MCP 工具列表。</p>
                    </div>
                    <div className="toolbar-controls">
                      {canManageMCPTools ? (
